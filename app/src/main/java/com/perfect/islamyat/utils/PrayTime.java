...
public class PrayTime {
    ...
    private double[] computeTimes(double[] times) {
        ...
        double Fajr = this.computeTime(
                190 - methodParams.get(this.getCalcMethod())[0], t[0]);
        Log.d("TAGlehhhh", "computeTimes: "+Fajr +" -- "+ (180-methodParams.get(this.getCalcMethod())[0] ));
...
//
//        prayers.setTimeFormat(prayers.TIME_24);
...
//    }
    ...
    public static void getPrayerTimes(Context context, int index, double lat, double lng, int timeFormat) {
        ...
        Calendar cal = Calendar.getInstance();
        Log.d("now = ", "getPrayerTimes: "+cal.getTime());
        System.out.println(dateFormat.format(cal.getTime()));//        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
//            dtf = DateTimeFormatter.ofPattern("HH:mm");
//            LocalTime now = LocalTime.now();
//            System.out.println(dtf.format(now));
//            Log.d("now = ", "getPrayerTimes: "+now);
//        }
        ...
        TimeZone defaultTz = TimeZone.getDefault();
        Log.d("TAGdefaultTz ", "getPrayerTimes: "+defaultTz.getID()+" - "+defaultTz.getDisplayName());
        //Get calendar object with current date/time
        Calendar defaultCalc = Calendar.getInstance(defaultTz);
        Log.d("TAGdefaultTz2 ", "getPrayerTimes: "+defaultCalc.getTime()+" - ");
        //Get offset from UTC, accounting for DST
        int defaultTzOffsetMs = defaultCalc.get(Calendar.ZONE_OFFSET) + defaultCalc.get(Calendar.DST_OFFSET);
        Log.d("TAGdefaultTz3 ", "getPrayerTimes: "+defaultTzOffsetMs+" - ");

        double timezone = defaultTzOffsetMs / (1000 * 60 * 60);

        Log.d("TAGdefaultTz4 ", "getPrayerTimes: "+timezone+" - ");
